...
class WarehousesTab(QWidget):
    ...
    Класс предоставляет интерфейс для просмотра, добавления, редактирования и удаления
    складов в системе. Также позволяет просматривать запасы на выбранном складе
    и экспортировать данные о складах в различных форматах.
    
    :наследует: :class:`PyQt6.QtWidgets.QWidget`
    """
    ...
    def __init__(self, db):
        ...
        :param db: Объект базы данных для операций со складами
        :type db: объект базы данных
        """
        super().__init__()
    ...
    def init_ui(self):
        ...
        Метод создает и настраивает все элементы интерфейса:
        - Верхнюю панель с кнопками управления и полем поиска
        - Таблицу для отображения информации о складах
    ...
    def load_warehouses(self):
        ...
        Метод выполняет запрос к базе данных для получения списка всех складов,
        заполняет таблицу полученными данными и обновляет счетчик количества складов.
    ...
    def handle_search(self):
        ...
        Метод получает текст из поля поиска, выполняет SQL-запрос для поиска складов
        по названию или местоположению и обновляет таблицу результатами поиска.
        В случае ошибки записывает сообщение в лог.
        """
        try:
    ...
    def add_warehouse(self):
        ...
        Метод создает и отображает диалог для добавления нового склада.
        При подтверждении диалога выполняет SQL-запрос для добавления 
        склада в базу данных и обновляет таблицу.
        """
    ...
    def delete_warehouse(self):
        ...
        Метод получает выбранный пользователем склад, проверяет отсутствие товаров на нем,
        запрашивает подтверждение на удаление и выполняет SQL-запрос для удаления склада.
        
        Если на складе есть товары, выводит предупреждение и отменяет удаление.
        """
    ...
    def edit_warehouse(self):
        ...
        Метод получает выбранный пользователем склад, загружает его данные,
        отображает диалог редактирования и выполняет SQL-запрос для обновления 
        информации о складе в базе данных.
        """
        # Получение выбранных строк
        selected_rows = self.warehouses_table.selectedItems()
        if not selected_rows:
    ...
    def show_warehouse_stock(self):
        ...
        Метод получает выбранный пользователем склад, загружает информацию
        о товарах на нем и отображает детальную информацию в диалоговом окне.
        Рассчитывает общее количество товаров и их суммарную стоимость.
        """
        # Получение выбранных строк
        selected_rows = self.warehouses_table.selectedItems()
        if not selected_rows:
    ...
    def export_data(self):
        ...
        Метод отображает диалог выбора параметров экспорта и экспортирует данные
        о складах в выбранном формате (Excel, CSV или PDF) по указанному пути.
        Учитывает текущий фильтр поиска при экспорте данных.
        """
